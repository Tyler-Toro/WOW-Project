...
def escaping_sanctum(user_name, user_weight, user_color, bravo_leader, user_item):
    ...
    if escaping_sanctum0 == "n" or escaping_sanctum0 == "N":
        ...
        main_lobby(user_name, user_weight, user_color, bravo_leader, user_item)
    else:
        print("This isn\'t a valid response. Try again:\n")
        escaping_sanctum(user_name, user_weight, user_color, bravo_leader, user_item)


def main_lobby(user_name, user_weight, user_color, bravo_leader, user_item):
...
staircase section at the end of the empty lobby. There is a sudden muffled sound that freezes {user_name} in their tracks\n")
    ...
    if escaping_sanctum2 == "1":
        time.sleep(quick)
        basement(user_color, user_name, bravo_leader, user_weight, user_item)
    elif escaping_sanctum2 == "2":
        time.sleep(quick)
        lab(user_name, user_color, user_weight, bravo_leader, user_item)
    elif escaping_sanctum2 == "3":
        time.sleep(quick)
        roof(user_name, roof_access, user_weight, bravo_leader, user_item)
...
def basement(user_color, user_name, bravo_leader, user_weight, user_item):
...
There is a directory on the wall\n")
    ...
    if basement_fork == "1":
        time.sleep(step)
        basement_unitB(user_name, user_color, user_weight, bravo_leader, user_item)
    elif basement_fork == "2":
        time.sleep(slow)
        basement_unitA(user_name, user_color, bravo_leader)
    else:
...
def basement_unitA(user_color, user_name, bravo_leader):
...
def basement_unitB(user_name, user_color, user_weight, bravo_leader, user_item):
...
def lab(user_name, user_color, user_weight, bravo_leader, user_item):
...
doors are blocked by a mountain of large boxes, lab equipment, and garbage\n")
    ...
    if lab_fork == "1":
        time.sleep(step)
        lab_onward(user_name, user_weight, user_color, bravo_leader, user_item)
    elif lab_fork == "2":
        time.sleep(slow)
        lab_flee(user_name, user_color, bravo_leader, user_weight, user_item)
    else:
        print("\n\t\tinvalid response".upper())
        time.sleep(step)
        print("\n\n")
...
def lab_onward(user_name, user_weight, user_color, bravo_leader, user_item):
...
def lab_flee(user_name, user_color, bravo_leader, user_weight, user_item):
    print(f"\n{user_name} guides Alpha Team back to the lobby and to the entrance of the main stairway\n")
    basement(user_color, user_name, bravo_leader, user_weight, user_item)


def roof(user_name, roof_access, user_weight, bravo_leader, user_item):
